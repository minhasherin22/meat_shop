...
class RdyToCook extends StatelessWidget {
   RdyToCook({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    var ListOfRdyCook = context.watch<ProductProvider>().readyToEatHome;
        ...
        children: [
          ...
          Expanded(
              child: GridView.builder(
                  itemCount: ListOfRdyCook.length,
                         ...
                         Stack(
                           children: [
                            ...
                            decoration: BoxDecoration(
                              image: DecorationImage(image:AssetImage(ListOfRdyCook[index].image))
                            ),
                           ),
                           Positioned(
                            left: 100,
                            ...
                            child: Container(
                              ...
                              child: ElevatedButton(
                                style: ElevatedButton.styleFrom(backgroundColor: tbclr2),
                                onPressed: (){
                                 Provider.of<ProductProvider>(context,listen: false).addtoCart(rdytocookList[index]);
                                },
                                 child: Icon(Icons.add,color: Colors.black,)),
                            ))
                           ]
                         ),
                        Text('${ListOfRdyCook[index].name}',style: font2,),
                        Text('${ListOfRdyCook[index].price}',style: font2,),
